    ...
    <xsl:template match="dap:Dataset">
        ...
        <xhtml>
            <head>
                ...
                <script type="text/javascript" src="{$serviceContext}/js/dap4_buttons.js"><xsl:value-of select="' '"/></script>
                ...
                <li>
                    ...
                    <ul>
                        <xsl:apply-templates select="./*[not(self::dap:Attribute)]">
                            <xsl:with-param name="container"><xsl:call-template name="computeVarName"/></xsl:with-param>
                        </xsl:apply-templates>    
                    </ul>
                </li>
            </ul>   
...
-->
    ...
    <xsl:template name="DimHeader">
        ...
        <xsl:for-each select="dap:Dim">
            ...
            <span class="medium">[<xsl:if test="@name"><span class="small" style="vertical-align: 15%;">
                <xsl:value-of select="$dimName"/>=</span></xsl:if>0<span class="medium" style="vertical-align: 10%;">..</span><xsl:value-of select="$dimSize - 1"/>]</span>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="DimSlicing">
        ...
        <xsl:for-each select="dap:Dim">
            ...
            <input type="text" id="{$dimTag}" size="8" oninput="autoResize(event)" onfocus="describe_index()" onChange="DAP4_URL.update_url()" />
            <script type="text/javascript">
    ...
    <xsl:template name="DimSize">
        <xsl:choose>
            ...
            <xsl:when test="starts-with(@name,'/')"><xsl:value-of select="key('DimensionNames', substring-after(@name,'/'))/@size"/></xsl:when>
    ...
    <xsl:template name="AttributeHeader">
        ...
        <div style="color: black;margin-left:-15px;margin-top:10px">
            <xsl:value-of select="@name"/>
            <span class="small" style="vertical-align: 15%; font-size: 25%;">: <xsl:value-of select ="name(.)"/>)</span>
        </div>
            ...
            <xsl:otherwise>
                <li>
                    <div class="small">
                        ...
                        <span class="em"><xsl:for-each select="dap:Value"> <xsl:value-of select="." /> </xsl:for-each></span>
