...
[[resources]]
== Resources
In the project directory you can find the resources included with the *_olfs_* when it's packaged for deployment. The default resources are located in _olfs/resources/hyrax_ these include js, jsp, xml, jnlp, and a directory (_olfs/resources/hyrax/xsl_) of XSL transform files that almost certainly contains some historical material that is not currently being utilized.

There are other directories with resources that may or may not be in use. The _olfs/resources/hyrax/ngap_ directory contains the resources that we use amend/replace things from the regular build to produce our NGAP builds. Things like a custom landing page and the libraries to let us use memcached to manage session state in our multi-host deployments.
