...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="bes:showNode">
        ...
        <xsl:variable name="topName" select="(bes:node/@name | bes:item/@name)"/>


        <xsl:variable name="catalogName">
            <xsl:choose>
                <xsl:when test="$topName='/'" >
                    <xsl:value-of select="@prefix"/>
                </xsl:when>
                <xsl:otherwise >
                    <xsl:value-of select="$besPrefix"/><xsl:value-of select="$topName"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <head>
                <link rel='stylesheet' href='{$docsService}/css/contents.css'
                      type='text/css'/>
                <title>OPeNDAP Hyrax: Contents of <xsl:value-of select="$topName"/></title>
            </head>
            <body>
                ...
                <table width="100%" >
                    <tr>
                        <td width="206px"><img alt="OPeNDAP Logo" src='{$docsService}/images/logo.svg'/></td>
                ...
                <h1><span class="small">meta for
                    <xsl:choose>
                        <xsl:when test="bes:item/@type='leaf'">leaf:</xsl:when>
                        <xsl:otherwise>node:</xsl:otherwise>
                    </xsl:choose>
                    </span> <xsl:value-of select="$besPrefix"/></h1>
                <table width="100%">
                    <tr>
                        <td class="small">
                            <xsl:if test="$topName!='/'" >
    ...
    <xsl:template name="ProcessCatalogLeaf"  >
        ...
        <table>
            <tr>
                <td class="medium" style="padding-right: 50px;"><xsl:value-of select="$item/@name" /></td>
                <td class="medium_bold"><a href="{$dapService}/{$fullName}"> Download </a></td>
            </tr>
            <tr>
                <td class="medium">Size (bytes)</td>
                <td class="medium"><xsl:value-of select="$item/@size" /></td>
            </tr>
            <tr>
                <td class="medium">Last Modified Time</td>
                <td class="medium"><xsl:value-of select="$item/@lastModified" /></td>
            </tr>
        </table>
