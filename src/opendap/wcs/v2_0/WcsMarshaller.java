	 ...
	 try
	 {
		...
		Dataset dataset = null;
 	    ...
 	    catch (Exception e)
 	    {
    		...
    		System.err.println("could not resolved into a DMR Dataset, please check access");
        ...
        net.opengis.wcs.v_2_0.CoverageDescriptionType cd = 
                new net.opengis.wcs.v_2_0.CoverageDescriptionType();
    	...
    	// it is obvious from method signature of setBoundedBy in CoverageDescription(cd) that 
    	// BoundingShapeType is needed as argument.  It is just an thin-wrapper to the more substantial 
    	// EnvelopwithTimePeriod object 
    	...
    	//bs.setEnvelope(new JAXBElement(new QName("http://www.opengis.net/gml/3.2", "EnvelopeWithTimePeriod"), envelope.getClass(),envelope));
		...
		offset1.withValue(0.5, 0.0); offset1.setSrsName("http://www.opengis.net/def/crs/EPSG/0/4326");
		offsetList.add(offset1);
		VectorType offset2 = gmlFactory.createVectorType();
		offset2.withValue(0.0, 0.625); offset2.setSrsName("http://www.opengis.net/def/crs/EPSG/0/4326");
    	...
    	//equivalent to: domainSet.setAbstractGeometry(new JAXBElement(new QName("http://www.opengis.net/gml/3.2", "RectifiedGrid"), rectifiedGrid.getClass(),rectifiedGrid));     	
        cd.setDomainSet(gmlFactory.createDomainSet(domainSet));
    	//equivalent to: cd.setDomainSet(new JAXBElement(new QName("http://www.opengis.net/gml/3.2", "domainSet"), domainSet.getClass(),domainSet));
        ...
        // NASA guys - Plus or Minus Fill values. Vmin, Vmax.  Reasonable gambit - to ask variable for max and min.  Say - give me range for hour, no rain?
        // get actual max and min.  http://testbed-13.opendap.org:8080/opendap/testbed-13/M2SDNXSLV.5.12.4/MERRA2_100.statD_2d_slv_Nx.19800101.SUB.nc4.ascii?range(HOURNORAIN)
        //  Dataset: function_result_MERRA2_100.statD_2d_slv_Nx.19800101.SUB.nc4
        // min, 0
        // max, 37800
        // is_monotonic, 0
        // there is no Vmax, what about Fill (NUG NetCDF USers Guide conventions for missing data...valid min, max, range)
        ...
        //dataRecord1Field.setAbstractDataComponent(new JAXBElement(new QName("http://www.opengis.net/swe/2.0", "Quantity"), dataRecord1FieldQuantity.getClass(),dataRecord1FieldQuantity));
        ...
        // Loop over all variables, ask two questions of rightmost two dimensions and the size of those left of the rightmost two dimensions
        // and if some criteria are true that particular variable can be a field in a coverage
        // Default: Rectified Grid - modulo one thing - lat long dimensions be monotonic and be evenly spaced... 
        // Lets punt...??..could be added to the Range Function...to tell us whether it is a rectified grid. 
        // There is cheesy way:compute the differences and say are they the same or not 
        serviceParameters.setCoverageSubtype(new QName("http://www.opengis.net/wcs/2.0", "RectifiedGridCoverage", "wcs"));
