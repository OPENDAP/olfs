...
The primary pattern used by the OLFS  for implementing services, _HttpServlet_ implementations that utilize _DispatchHandler_ implementations to claim requests and respond to them. In some cases a _servlet_ might utilize a single instance of DispatchHandler to run the show. For example, _opendap.build_dmrpp.BuildDmrppServlet_ relies solely on _opendap.build_dmrpp.BuildDmrppDispatchHandler_ and _opendap.wcs.v2.0.Servlet_ relies solely on _opendap.wcs.v2.0.HttpGetHandler_. In other examples there are many _DispatchHandler_ in the Servlet. For example _opendap.coreServlet.DispatchServlet_ has about a dozen different _DispatchHandler_ instances.


== How? How does it work?

Let's look at the primary servlet, _opendap.coreServlet.DispatchServlet_, handles an incoming request:
* In the doGet() method the code first handles some bookkeeping (such as producing a request id and starting the access log for the request) before iterating over its internal vector of DispatchHandlers, asking each if it can service the request. The first DispatchHandlers to claim the request is then asked to service it and send the response. The order of the DispatchHandler objects in the vector is critical, as there are greedy DispatchHandlers (FileDispatchHandler for example) that will claim things that should/would have been claimed by a different DispatchHandler. The order is established in the init() method.

If the request is for some type of DAP response then the BesDapDispatcher will claim it.

Inside BesDapDispatcher there is a vector of Dap4Responder objects, each one associated with a particular DAP response type (such as: use DAP2 and return a netcdf-3 encoded response, use DAP4 and return the DMR response, etc.) The BesDapDispatcher iterates over its vector of Dap4Responders and hands the response duties to the first one that claims the request.

So how does all this claiming work? Through the magic of regular expressions. Each of the Dap4Responders contains a primary regular expression which  should match a URL path in the request. For example the NormativeDMR responder has something like this `^.*\.dmr$`. But it's more complex because in DAP4 there are several different alternate encodings for the DMR (and the other response types) that a client might wish to receive. This is handled by having each normative responder hold a collection of alternative responders, each of which has it's own suffix and regex. The NormativeDMR will combine all of these so that when it is asked if a request can be handled it simply evaluates the request URL path to see if it matches the combined regex of its own regex and all of it alternative responder regexs. When the request is submitted for processing NormativeDMR locates the appropriate responder and sends the request to it.
