...
public class ArchiveDownload implements  Serializable {
    ...
    private InitiateJobResult _initiateJobResult;
    private GlacierRecord _glacierRecord;
    ...
    public ArchiveDownload(String vaultName, String glacierEndpointUrl, AWSCredentials awsCredentials, long expectedDelay) {
    ...
    ArchiveDownload(GlacierRecord glacierRecord, AWSCredentials awsCredentials, long expectedDelay) {

        this(glacierRecord.getVaultName(),GlacierArchiveManager.theManager().getGlacierEndpoint(),awsCredentials,expectedDelay);
    ...
    public boolean startArchiveRetrieval() throws IOException {

        _log.debug("startArchiveRetrieval() - BEGIN ");
        ...
        try {
            ...
            _log.debug("startArchiveRetrieval() - Glacier download job started. jobId: {}",_initiateJobResult.getJobId());

            _started = true;

        }
        catch (Exception e) {
            _log.error("startArchiveRetrieval() - Download failed to start! msg: {}",e.getMessage());
        }

        _log.debug("startArchiveRetrieval() - END ");
    ...
    public boolean isReadyForDownload() throws IOException {
        ...
        if(!_started)
            throw new IOException("Glacier retrieval job for resourceID '"+_glacierRecord.getResourceId()+" has NOT been started!");
