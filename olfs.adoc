...
* *Browser Navigable Catalogs*: The OLFS uses BES showNode commands to discover what the BES "catalog" (usually a filesystem but maybe not) contains. The BES responses are XML documents that the OLFS operates on with an XSL transform to produce http://test.opendap.org:8080/opendap/"[the "blue bar" browser navigation pages that Hyrax produces]. Some parts of this representaion are controlled by the *_olfs.xml_* file used with the deployment. (See <<configuration>>)

* *Machine Readable Catalogs* _No Scraping Required!_
